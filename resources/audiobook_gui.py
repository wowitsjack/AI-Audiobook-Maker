...
Created with ❤️ for audiobook enthusiasts""")
    ...
    def preview_background_music(self):
        ...
        try:
            ...
            from music_generator import MusicGenerator, MusicConfig, MusicMood, MusicGenre
...
💡 Brightness: {self.music_brightness.get():.1f}"""
    ...
    def process_file_with_chunking(self, filepath):
        ...
        try:
            ...
            if not self.enable_chunking.get() or word_count <= threshold:
                # Always use .txt extension for chunk filename to ensure proper .wav conversion
                chunk_filename = f"{name_without_ext}.txt"
                if not self.enable_chunking.get() and word_count > threshold:
                    return [(chunk_filename, content, f"{name_without_ext} (Complete - {word_count} words)")]
                else:
                    return [(chunk_filename, content, f"{name_without_ext} ({word_count} words)")]
            else:
    ...
    def generate_audiobook(self, custom_prompt):
        ...
        try:
            ...
            # Use the same prompt format as app.py - just the custom prompt as a style instruction
            # This will be passed to generate_chapter_audio which formats it correctly as: "{custom_prompt}: {content}"
            combined_instructions = custom_prompt
